...
2. Power BI Desktop에서 사용는 것과 동일한 계정으로 로그인 프로세스를 완료합니다.
...
1. Power BI 서비스에서 작업 영역을 만들려면 왼쪽에 있는 **탐색** 창에서 **작업 영역**을 선택한 다음 **+ 새 작업 영역**을 선택합니다.

2. 오른쪽에 있는 **작업 영역 만들기** 창의 **이름** 상자에 작업 영역의 이름을 입력합니다.
...
2. 작업 영역 방문 페이지에 **Sales Analysis - Enforce model security** 의미 체계 모델이 있습니다.
...
7. 역할 매핑을 완료하려면 **저장**을 선택합니다.
    ...
    *이 디자인 방식은 각 지역에 보안 그룹이 존재하는 경우 간단하고 효과적입니다. 그러나 만들기 및 설정에 더 많은 노력이 필요하다는 단점이 있습니다. 또한 새 지역이 온보딩될 때 의미 체계 모델을 업데이트하고 다시 게시해야 합니다.*
...
1. 의미 체계 모델을 제거하려면 의미 체계 모델 위로 커서를 가져가서 줄임표가 나타나면 줄임표를 선택한 다음 **삭제**를 선택합니다.

    ![](Images/enforce-model-security-image37.png)

    *다음 연습에서 수정된 의미 체계 모델을 다시 게시합니다.*
...
6. **테이블 필터 DAX 표현식** 상자에서 **“값”** 을 `USERPRINCIPALNAME()`로 바꿉니다.
...
2. **역할로 보기** 창에서 **다른 사용자** 항목을 선택하고 해당 상자에 `michael9@adventure-works.com`을 입력합니다.
