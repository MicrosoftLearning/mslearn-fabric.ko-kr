...
1. 기본 쿼리 편집기에서 **변수 이름**을 `base_bike_data`(으)로 설정하고 **자전거 대여 데이터** 원본이 선택되어 있는지 확인합니다. 다음 쿼리를 입력합니다.
    ...
    bikes
        ...
        | summarize latest_observation = arg_max(ingestion_time(), *) by Neighbourhood
    ```
1. 쿼리를 실행하고 대시보드의 시각적 개체(및 다른 시각적 개체)에 필요한 모든 열을 반환했는지 확인합니다.

   ![기본 쿼리의 스크린샷.](./Images/dashboard-base-query.png)

1. **완료**를 선택한 다음 **기본 쿼리** 창을 닫습니다.
1. **자전거 및 도크** 막대형 차트 시각적 개체를 편집하고 쿼리를 다음 코드로 변경합니다.
