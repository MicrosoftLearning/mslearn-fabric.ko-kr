...
1. **대상 테이블 선택 또는 만들기** 창에서 `stock`(이)라는 새 테이블을 만듭니다. 그런 다음 **데이터 원본 구성** 창에서 작업 영역과 **stock-data** 이벤트 스트림을 선택하고 연결 이름을 `stock-data`(으)로 지정합니다.
...
1. 왼쪽 메뉴 모음에서 **실시간** 허브를 선택한 다음 **내 데이터 스트림** 페이지를 확인합니다. **주식** 테이블과 **stock-data-stream** 스트림이 나열되어야 합니다.

   ![실시간 허브의 내 스트림 페이지 스크린샷.](./Images/my-data-streams.png)

1. **stock-data-stream**스트림의 **...** 메뉴에서, **이벤트 스트림 열기**를 선택합니다.
