...
이 랩에서는 당뇨병의 정량적 측정값을 예측하는 기계 학습 모델을 학습시킵니다. scikit-learn을 사용하여 회귀 모델을 학습시키고 모델을 MLflow와 추적하고 비교합니다.

이 랩을 완료하면 기계 학습 및 모델 추적에 대한 실습 경험을 얻고 Microsoft Fabric에서 Notebook *, *실험* 및 *모델을* 사용하는 *방법을 알아봅니다.
...
모델을 학습하려면 Notebook*을 *만들 수 있습니다. Notebook은 코드를 작성하고 실행할 수 있는 대화형 환경을 제공합니다(여러 언어로).
...
이제 코드를 실행하여 데이터를 가져와 모델을 학습할 준비가 되었습니다. Azure Open Datasets의 [당뇨병 데이터 세트](https://learn.microsoft.com/azure/open-datasets/dataset-diabetes?tabs=azureml-opendatasets?azure-portal=true) 로 작업합니다. 데이터를 로드한 후에는 데이터를 Pandas 데이터 프레임으로 변환합니다. 즉, 행과 열에서 데이터를 사용하기 위한 일반적인 구조입니다.
...
이제 데이터를 로드했으므로 이를 사용하여 기계 학습 모델을 학습시키고 당뇨병의 정량적 측정값을 예측할 수 있습니다. scikit-learn 라이브러리를 사용하여 회귀 모델을 학습시키고 MLflow를 사용하여 모델을 추적합니다.
...
1. Notebook에 다른 새 코드 셀을 추가하고, 여기에 다음 코드를 입력하고, 실행합니다.
   ...
   mlflow.set_experiment(experiment_name)
    ...
    이 코드는 실험 당뇨병이라는 **MLflow 실험을 만듭니다**. 모델은 이 실험에서 추적됩니다.
...
1. Notebook에 다른 새 코드 셀을 추가하고, 여기에 다음 코드를 입력하고, 실행합니다.
   ...
   with mlflow.start_run():
    ...
    이 코드는 선형 회귀를 사용하여 회귀 모델을 학습합니다. 매개 변수, 메트릭 및 아티팩트가 MLflow를 사용하여 자동으로 기록됩니다. 또한 LinearRegression 값*으로 추정기**라는 **매개 변수를 로깅합니다*.
...
1. Notebook에 다른 새 코드 셀을 추가하고, 여기에 다음 코드를 입력하고, 실행합니다.
   ...
   with mlflow.start_run():
    ...
    이 코드는 의사 결정 트리 회귀기를 사용하여 회귀 모델을 학습합니다. 매개 변수, 메트릭 및 아티팩트가 MLflow를 사용하여 자동으로 기록됩니다. 또한 DecisionTreeRegressor 값을 *사용하여 추정기**라는 **매개 변수를 로깅합니다*.
...
1. 작업 실행 및 출력을 보다 쉽게 비교하려면 검색을 구성하여 결과를 정렬할 수 있습니다. 예를 들어 다음 셀은 결과를 *start_time* 정렬하고 최대 2개의 결과만 표시합니다.
...
1. 마지막으로 여러 모델의 평가 메트릭을 나란히 그리면 모델을 쉽게 비교할 수 있습니다.
   ...
   plt.show()
    ...
    ![표시된 평가 메트릭의 스크린샷.](./Images/data-science-metrics.png)
...
1. 저장 실행 모델** 상자에서 **저장**을 선택합니다**(이를 보려면 오른쪽으로 스크롤해야 할 수 있음).
1. 새로 연 팝업 창에서 새 모델** 만들기를 선택합니다**.
1. 모델** 폴더를 **선택합니다.
1. 모델 `model-diabetes`이름을 지정하고 저장**을 선택합니다**.
1. 모델을** 만들 때 화면 오른쪽 위에 표시되는 알림에서 ML 모델 보기를 선택합니다**. 창을 새로 고칠 수도 있습니다. 저장된 모델은 모델 버전**에서 **연결됩니다.
...
1. 전자 필기장으로 돌아가서 전자 필기장 메뉴 모음에서 설정** 아이콘을 사용하여 ⚙️ **전자 필기장 설정을 봅니다.
