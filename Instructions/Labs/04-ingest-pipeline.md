...
5. Notebook이 이와 비슷한지 확인한 다음 **,&#9655; 도구 모음의 모든** 단추를 실행하여 포함된 모든 셀을 실행합니다.
    ...
    > **참고**: 이 세션에서 Spark 코드를 실행한 것은 이번이 처음이기 때문에 Spark 풀이 시작되어야 합니다. 즉, 첫 번째 셀을 완료하는 데 1분 정도 걸릴 수 있습니다.

6. Notebook 실행이 완료되면 왼쪽의 **Lakehouse 탐색기** 창에서 **테이블**의 **...** 메뉴에서 새로 고침**을 선택하고 **판매** 테이블이 **만들어졌는지 확인합니다.
7. 전자 필기장 메뉴 모음에서 ⚙️ **설정** 아이콘을 사용하여 전자 필기장 설정을 봅니다. 그런 다음, Notebook의 이름을** Load Sales**로 **설정하고 **설정 창을 닫습니다.
