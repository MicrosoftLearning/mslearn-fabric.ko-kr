...
1. 왼쪽 창의 **레이크 보기** 탭에 있는 **파일** 노드의 **...** 메뉴에서 **새 하위 폴더**를 선택하고 **new_data**라는 하위 폴더를 만듭니다.
...
2. **데이터 복사** 마법사가 자동으로 열리지 않으면 파이프라인 편집기 페이지에서 **데이터 복사**를 선택합니다.
3. **데이터 복사** 마법사의 **데이터 원본 선택** 페이지에 있는 **데이터 원본** 섹션에서 **제네릭 프로토콜** 탭을 선택하고 그런 다음 **HTTP**를 선택합니다.

    ![데이터 원본 선택 페이지의 스크린샷.](./Images/choose-data-source.png)

4. **다음**을 선택한 후 **새 연결 만들기**를 선택하고 데이터 원본에 대한 연결에 대해 다음 설정을 입력합니다.
    - **URL**: `https://raw.githubusercontent.com/MicrosoftLearning/dp-data/main/sales.csv`
    - **연결**: 새 연결 만들기
    - **연결 이름**: *고유 이름 지정*
    - **데이터 게이트웨이**:(없음)
    - **인증 종류**: 기본(*사용자 이름과 암호를 비워 둠*)
...
8. **데이터 대상에 연결** 페이지에서 기존 레이크하우스를 선택합니다. 그런 후 **다음**을 선택합니다.
...
2. **작업** 탭의 **작업 자세히** 목록에서 **데이터 삭제**를 선택합니다. 그런 다음 새 **데이터 삭제** 작업을 **데이터 복사** 작업 왼쪽에 배치하고 다음과 같이 **완료 시** 출력을 **데이터 복사** 작업에 연결합니다.
