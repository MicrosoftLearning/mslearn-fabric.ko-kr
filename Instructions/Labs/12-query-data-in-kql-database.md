---
lab:
  title: Microsoft Fabric에서 KQL 데이터베이스 쿼리 시작
  module: Query data from a KQL database in Microsoft Fabric
---

# Microsoft Fabric에서 KQL 데이터베이스 쿼리 시작
...
이 연습에서는 NYC 택시 승차 데이터의 데이터 세트를 쿼리하는 임무를 맡은 분석가의 역할을 수행하게 됩니다. KQL을 사용하여 이 데이터를 쿼리하고 정보를 수집하여 데이터에 대한 정보 인사이트를 얻습니다.

> **팁**: KQL 쿼리 세트는 많은 SQL 함수와 호환되는 Kusto 쿼리 언어를 사용하여 쿼리를 만듭니다. KQL에 대해 자세히 알아보려면 [KQL(Kusto 쿼리 언어) 개요](https://learn.microsoft.com/azure/data-explorer/kusto/query/?context=%2Ffabric%2Fcontext%2Fcontext)를 참조합니다.
...
1. 이벤트 하우스에서 만든 KQL 데이터베이스에 대한 **...** 메뉴에서 데이터**데이터 가져오기** > **샘플**을 선택합니다. 그런 다음 **Automotive operations analytics** 샘플 데이터를 선택합니다.

1. 데이터 로드가 완료되면 **Automotive** 테이블이 만들어졌는지 확인합니다.
...
1. **sum** 함수를 사용하여 여 데이터를 요약하여 총 이동한 마일 수를 확인하는 다음 쿼리를 시도합니다.
    ...
    요약된 데이터를 지정된 열 또는 식으로 그룹화할 수 있습니다.
