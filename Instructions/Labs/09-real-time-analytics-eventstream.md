...
5. Power BI 포털 왼쪽 하단에서 **Power BI** 아이콘을 선택하고 **실시간 분석** 환경으로 전환합니다.
...
실시간 분석 스트리밍 및 쿼리 기능을 사용하여 주식 통계에 대한 주요 질문에 답변합니다. 이 시나리오에서는 KQL 데이터베이스와 같은 일부 구성 요소를 독립적으로 수동으로 만드는 대신 마법사를 최대한 활용할 예정입니다.
...
3. 이제 **새 대상**을 선택한 다음 **KQL 데이터베이스**를 선택하여 대상을 추가합니다.
...
모든 단계가 녹색 확인 표시로 표시될 때까지 기다립니다. 페이지 제목이 **Eventsream에서 지속적인 수집이 설정됨**인지 확인해야 합니다. 그런 다음 **닫기**를 선택하여 Eventstream 페이지로 돌아갑니다.
...
1. **MyStockData**라는 이름으로 새로 만들어지고 하이드레이션된 KQL 데이터베이스를 찾습니다.
2. 데이터 트리에서 MyStockData 테이블의 추가 메뉴 [...]를 선택합니다. 그런 다음 쿼리 테이블 > 100개 레코드 표시를 선택합니다.
...
4. 데이터 트리로 돌아가서 where 연산자와 between 연산자를 사용하여 지난 24시간 동안 수집된 레코드를 반환하는 다음 쿼리를 선택합니다.
...
기본 제공 함수를 사용하여 계속 탐색하고 SQL 또는 KQL을 사용하여 데이터에 익숙해질 수 있습니다. 이제 수업이 종료됩니다.
