...
4. **새 Eventstream**의 **이름을 지정**하고 **향상된 기능(미리 보기)** 옵션을 선택한 다음 **만들기** 단추를 선택합니다.
...
1. **추가** 단추를 선택하면 스트림이 매핑되고 자동으로 **Eventstream 캔버스**로 리디렉션됩니다.
...
3. 다음 표에 표시된 대로 샘플 데이터 값을 입력한 다음 **추가**를 선택합니다.
...
1. **Eventstream** 캔버스 내에서 **이벤트 변환**을 선택합니다.
    ...
    C. **그룹화 기준** 설정 섹션의 속성 작성

    [ ![변환 이벤트에 그룹화 기준을 추가합니다.](./Images/eventstream-add-aggregates.png) ](./Images/eventstream-add-aggregates-large.png) 

2. **그룹화 기준** 변환 이벤트를 만든 후에는 이를 **Eventstream**에서 **그룹화 기준**으로 연결해야 합니다. **Eventstream** 오른쪽에 있는 점을 클릭하고 새 **group by** 상자 왼쪽에 있는 점으로 끌면 코드를 사용하지 않고도 이 작업을 수행할 수 있습니다.

   [ ![Eventstream과 그룹화 기준 사이에 링크를 추가합니다.](./Images/group-by-drag-connectors.png) ](./Images/group-by-drag-connectors-large.png) 

3. 같은 방법으로 **이벤트 스트림**과 ***kql_dest*** 사이의 화살표 위에 마우스를 놓고 ***휴지통***을 선택할 수 있습니다.

   [ ![두 이벤트 간의 링크 제거](./Images/delete-flow-arrows.png) ](./Images/delete-flow-arrows-large.png)

    > 커넥터를 추가하거나 제거할 때마다 대상 개체를 다시 구성해야 합니다.
...
4. 데이터 트리로 돌아가서 요약 연산자를 사용하여 15분 간격으로 수집된 레코드 수를 계산하는 다음 쿼리를 선택합니다.
