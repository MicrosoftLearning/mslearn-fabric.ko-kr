...
이 랩에서는 기계 학습 모델을 사용하여 당뇨병의 정량적 측정값을 예측합니다.

이 랩을 완료하면 예측을 생성하고 결과를 시각화하는 실습 경험을 얻을 수 있습니다.
...
1. 브라우저에서 Microsoft Fabric 홈페이지 `https://app.fabric.microsoft.com` 로 이동합니다.
1. Microsoft Fabric 홈페이지**에서 Synapse 데이터 과학**
...
첫째, 회귀* 알고리즘을 사용하여 *당뇨병 환자의 관심 반응을 예측하는 기계 학습 모델을 학습해 보겠습니다(기준선 이후 1년 후 질병 진행의 정량적 측정값).
...
1. 셀 출력 아래 + **코드** 아이콘을 사용하여 Notebook에 새 코드 셀을 추가하고 다음 코드를 입력하여 이전 셀의 실험에서 학습한 모델을 등록합니다.
   ...
   print("Version: {}".format(mv.version))
    ...
    이제 모델이 작업 영역에 **당뇨병 모델로** 저장됩니다. 필요에 따라 작업 영역에서 찾아보기 기능을 사용하여 작업 영역에서 모델을 찾고 UI를 사용하여 탐색할 수 있습니다.

## Lakehouse에서 테스트 데이터 세트 만들기

모델을 사용하려면 당뇨병 진단을 예측해야 하는 환자 세부 정보의 데이터 세트가 필요합니다. 이 데이터 세트는 Microsoft Fabric Lakehouse에서 테이블로 만듭니다.

1. Notebook 편집기의 왼쪽 탐색기 창에서 **+ 데이터 원본을 선택하여 **레이크하우스를 추가합니다**.**
1. 새 레이크하우스**를 선택하고 **추가**를 선택하고 **원하는 올바른 이름으로 새 **Lakehouse**를 만듭니다.
1. 현재 세션을 중지하라는 메시지가 표시되면 지금** 중지를 선택하여 **Notebook을 다시 시작합니다.
1. Lakehouse가 만들어지고 Notebook에 연결된 경우 다음 코드를 실행하는 새 코드 셀을 추가하여 데이터 세트를 만들고 레이크하우스의 테이블에 저장합니다.
...
1. 코드가 완료되면 **Lakehouse 탐색기** 창의 **테이블** 옆에 있는 **...** 를 선택하고 새로 고침**을 선택합니다**. **diabetes_test** 테이블이 나타납니다.
1. 왼쪽 창에서 **diabetes_test** 테이블을 확장하여 포함된 모든 필드를 봅니다.

## 모델을 적용하여 예측 생성

이제 이전에 학습한 모델을 사용하여 테이블의 환자 데이터 행에 대한 당뇨병 진행 예측을 생성할 수 있습니다.
...
1. 코드가 완료되면 **Lakehouse 탐색기** 창에서 **diabetes_test** 테이블 옆에 있는 **...** 를 선택하고 새로 고침**을 선택합니다**. 새 필드 **예측이** 추가되었습니다.
1. Notebook에 새 코드 셀을 추가하고 diabetes_test** 표를 끌어 **놓습니다. 테이블의 내용을 보는 데 필요한 코드가 표시됩니다. 셀을 실행하여 데이터를 표시합니다.

## 리소스 정리

이 연습에서는 모델을 사용하여 일괄 처리 예측을 생성했습니다.

Notebook 탐색을 완료한 경우 이 연습에 대해 만든 작업 영역을 삭제할 수 있습니다.
